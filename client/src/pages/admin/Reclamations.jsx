...
export default function AdminReclamations() {
  ...
  const handleTraiter = async (id) => {
    try {
      await api.put(`/reclamations/${id}`, { status, reponse });
  ...
  return (
    <div className="app-shell">
      ...
      <div className="main-content">
        <Topbar title="Gestion des réclamations" />
        <div className="page-content">
          <div className="table-card">
            <div className="table-card-header">
              <h5><i className="fas fa-exclamation-circle me-2"></i>Réclamations ({reclamations.length})</h5>
            </div>
            <div className="table-responsive">
              <table className="table table-hover mb-0">
                <thead style={{ background: '#f8f9fa' }}>
                  <tr><th>#</th><th>Client</th><th>Type</th><th>Description</th><th>Date</th><th>Statut</th><th>Actions</th></tr>
                </thead>
                <tbody>
                  ...
                  reclamations.map((r, i) => (
                    <tr key={r._id}>
                      ...
                      <td>
                        <span className={r.status === 'Approuvée' ? 'badge-active' : r.status === 'Rejetée' ? 'badge-expired' : 'badge-attente'}>
                          {r.status}
                        </span>
                      </td>
                      <td>
                        {r.status === 'En attente' && (
                          <button className="btn btn-sm btn-outline-primary" onClick={() => { setSelected(r); setReponse(''); setStatus('Approuvée'); }}>
                            <i className="fas fa-reply"></i> Traiter
                          </button>
                        )}
                      </td>
                    </tr>
      ...
      {selected && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'white', borderRadius: 12, padding: 30, width: '100%', maxWidth: 500 }}>
            <h5 style={{ color: '#006652', marginBottom: 20 }}>Traiter la réclamation</h5>
            ...
            <div className="form-group">
              <label>Réponse</label>
              <textarea className="form-control-custom" rows={4} value={reponse} onChange={e => setReponse(e.target.value)} placeholder="Votre réponse au client..."></textarea>
            </div>
