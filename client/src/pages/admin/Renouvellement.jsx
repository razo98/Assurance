...
export default function AdminRenouvellement() {
  const [search, setSearch] = useState('');
  const [results, setResults] = useState([]);
  const [searched, setSearched] = useState(false);
  const [loading, setLoading] = useState(false);
  const [modal, setModal] = useState(null); // assurance sélectionnée
  ...
  const handleSearch = async e => {
    ...
    try {
      const { data } = await api.get(`/assurances?search=${encodeURIComponent(search)}`);
  ...
  const handleRenouveler = async e => {
    ...
    try {
      ...
      setSuccess(`Contrat renouvelé avec succès ! Nouveau prix TTC : ${data.prix_ttc?.toLocaleString()} FCFA`);
      setModal(null);
      // Retirer la carte renouvelée
  ...
  const canRenew = a => {
    ...
    const now = new Date();
    const diff = (fin - now) / (1000 * 60 * 60 * 24);
    return diff < 360; // peut renouveler si expire dans moins d'un an
  };
    ...
    <div className="app-shell">
        ...
        <div className="page-content">
          ...
          {searched && !loading && (
            results.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '50px 20px', background: 'white', borderRadius: 14, boxShadow: '0 3px 12px rgba(0,0,0,0.07)' }}>
                ...
                <p style={{ color: '#aaa', fontSize: 14 }}>Vérifiez l'immatriculation ou l'identifiant.</p>
              </div>
            ) : (
                        ...
                        <div style={{ padding: '16px 18px' }}>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 16px', marginBottom: 14 }}>
                            {[
                              ...
                              ['Puissance', `${a.puissance} CV`],
                              ['Prix TTC', `${(a.prix_ttc || 0).toLocaleString()} FCFA`],
                            ].map(([k, v]) => (
                              <div key={k}>
                                <div style={{ fontSize: 11, color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{k}</div>
                                <div style={{ fontWeight: 600, fontSize: 13.5, color: '#333' }}>{v}</div>
                              </div>
                            ))}
                          </div>
                          ...
                          <button
                            ...
                            style={{
                              ...
                              color: ok ? 'white' : '#aaa', fontWeight: 700, fontSize: 14,
                              cursor: ok ? 'pointer' : 'not-allowed',
                              transition: 'all 0.2s'
                            }}
                          >
                            <i className="fas fa-sync-alt me-2"></i>
                            {ok ? 'Renouveler ce contrat' : 'Renouvellement non disponible'}
                          </button>
                        </div>
                      </div>
                    </div>
          ...
          {!searched && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <p style={{ color: '#888', fontSize: 16 }}>Entrez une immatriculation pour commencer la recherche</p>
            </div>
          )}
        </div>
      </div>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(3px)' }}>
          <div style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 480, boxShadow: '0 24px 60px rgba(0,0,0,0.3)', animation: 'scaleIn 0.28s ease' }}>
            <div style={{ background: 'linear-gradient(to right, #004d3d, #006652)', color: 'white', padding: '18px 24px', borderRadius: '16px 16px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontWeight: 800, fontSize: 17 }}>Renouveler l'assurance</div>
                <div style={{ fontSize: 13, opacity: 0.8 }}>{modal.immatriculation}</div>
              </div>
              <button onClick={() => setModal(null)} style={{ background: 'none', border: 'none', color: 'white', fontSize: 22, cursor: 'pointer', opacity: 0.7 }}>×</button>
            </div>
            <form onSubmit={handleRenouveler} style={{ padding: '24px' }}>
              {error && <div className="alert-custom alert-error mb-3"><i className="fas fa-exclamation-circle me-2"></i>{error}</div>}

              <div style={{ background: '#f0f4f0', borderRadius: 10, padding: '12px 16px', marginBottom: 20, fontSize: 13.5 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px' }}>
                  <div><span style={{ color: '#888' }}>Client :</span> <strong>{modal.clients || '-'}</strong></div>
                  <div><span style={{ color: '#888' }}>Immat. :</span> <strong>{modal.immatriculation}</strong></div>
              ...
              <div style={{ background: '#e8f5f1', borderRadius: 8, padding: '10px 14px', marginBottom: 20, fontSize: 13, color: '#006652' }}>
                <i className="fas fa-info-circle me-2"></i>
                Le prix sera recalculé selon la nouvelle durée. Le contrat sera activé directement.
              </div>

              <div style={{ display: 'flex', gap: 10 }}>
                <button type="button" onClick={() => setModal(null)} style={{ flex: 1, padding: '12px', border: '2px solid #ddd', background: 'white', borderRadius: 9, cursor: 'pointer', fontWeight: 700, fontSize: 14 }}>
                  Annuler
                </button>
                <button type="submit" disabled={submitting} style={{ flex: 2, padding: '12px', background: submitting ? '#aaa' : 'linear-gradient(to right, #006652, #008a6e)', border: 'none', color: 'white', borderRadius: 9, cursor: submitting ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: 14 }}>
                  {submitting ? <><span className="spinner me-2"></span>Traitement...</> : <><i className="fas fa-sync-alt me-2"></i>Confirmer le renouvellement</>}
                </button>
              </div>
            </form>
          </div>
