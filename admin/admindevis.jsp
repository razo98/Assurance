...
<%
    ...
    if(adminId == null || role == null || !"soleil".equals(role)){
...
<head>
  <meta charset="UTF-8">
  <title>Demande de Devis Admin</title>
...
<body>
  ...
  <div class="container">
    ...
    <div class="user-badge">
      <i class="fas fa-user-shield"></i> <%= fullName %>
    </div>
    
    <div class="form-container">
      <h2>Nouvelle Assurance (Admin)</h2>
      
      <!-- Affichage de l'admin connecté -->
      <div class="user-info">
        <i class="fas fa-id-badge"></i>
        <div>
          <p><strong>Admin :</strong> <%= adminFirstName %> <%= adminLastName %></p>
          <p><small><strong>Rôle :</strong> <%= role %></small></p>
        </div>
      </div>
      
      <!-- Formulaire de création d'assurance -->
      <form id="assuranceForm" action="admintraitement.jsp" method="post">
            ...
            <div class="form-group">
              <select name="id_voiture" id="id_voiture" class="form-select" required>
                ...
                <% while(rsVoiture.next()){ %>
                  <option value="<%= rsVoiture.getInt("id_voiture") %>"><%= rsVoiture.getString("marque") %></option>
                <% } %>
              </select>
            ...
            <div class="form-group">
              <select name="id_categorie" id="id_categorie" class="form-select" required>
                ...
                <% while(rsCategorie.next()){ %>
                  <option value="<%= rsCategorie.getInt("id_categorie") %>"><%= rsCategorie.getString("genre") %></option>
                <% } %>
              </select>
...
<%
    ...
    if(rsVoiture != null) rsVoiture.close();
    if(rsCategorie != null) rsCategorie.close();
